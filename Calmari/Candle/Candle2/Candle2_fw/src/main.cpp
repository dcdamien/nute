...
void CC_t::Task() {
    if (NewPacketReceived) {    // Load data from FIFO
        ...
        if (FifoSize <= CC_PKT_DATA_LENGTH+2) NewPacketReceived = false;    // Otherwise there is more than one packet in the buffer
    }
