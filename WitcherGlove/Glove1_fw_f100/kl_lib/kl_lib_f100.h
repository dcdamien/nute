...
// Timer2 remap
#define T2_PA0_PA1_PA2_PA3      0x0000
#define T2_PA15_PB3_PA2_PA3     0x0100
#define T2_PA0_PA1_PB10_PB11    0x0200
#define T2_PA15_PB3_PB10_PB11   0x0300
static inline void Tim2Remap(uint32_t RemapMask) {
    RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;     // Enable AFIO
    uint32_t tmp = AFIO->MAPR;
    tmp &= ~0x00000300;
    tmp |= RemapMask;
...
private:
    USART_TypeDef *IUart;
    const stm32_dma_stream_t *PDMAStream;
...
public:
    ...
    void Init(USART_TypeDef *PUart, uint32_t ABaudrate);
