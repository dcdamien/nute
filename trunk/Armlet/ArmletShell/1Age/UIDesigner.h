...
#define USERNAME_WIDTH_PX  SCREENX - FORCESSTATUS_WIDTH_PX - BATTERYSTATUS_WIDTH_PX - TIME_WIDTH_PX
	...
	//frmMain
		...
		IControl* _pnlMainFormControls[2];
			...
			IControl* _pnlStatusBarControls[4];
				...
				//txtRoomId
				char _txtTimeBuff[TIME_BUFF_WIDTH*TIME_BUFF_HEIGHT];
				
				//txtForce
				char _txtForce[FORCE_BUFF_WIDTH*FORCE_BUFF_HEIGHT];
...
//				char _strMainFormSignal[MAINFORM_TEXT_SIGNAL_BUFF_WIDTH*MAINFORM_TEXT_SIGNAL_BUFF_HEIGHT];
	...
	//frmConsForm
		...
		IMenuItem* _mnuConsMenuItems[CONS_COUNT];
		MenuItem _miCons[CONS_COUNT];
		TextField _txtConsMenu[CONS_COUNT];
		char _bufConsMenu[CONS_COUNT][16];
