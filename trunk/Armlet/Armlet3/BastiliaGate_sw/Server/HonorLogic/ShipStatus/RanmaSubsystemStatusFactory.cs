...
namespace HonorLogic.ShipStatus
{
    public static class RanmaSubsystemStatusFactory
    {
        ...
        private static List<ushort> mediumTables = new List<ushort>() { };
        private static List<ushort> hardTables = new List<ushort>() { };
        public static RanmaSubsystemStatus GenerateRanmaSubsystemStatus(RanmaRepairSeverity severity)
        {
            ...
            switch (severity)
            {
                    ...
                    case RanmaRepairSeverity.Medium:
                        result = new RanmaSubsystemStatus(mediumTables[random.Next(easyTables.Count - 1)]);
                        break;
                    case RanmaRepairSeverity.Hard:
                        result = new RanmaSubsystemStatus(hardTables[random.Next(easyTables.Count - 1)]);
                        break;
                default:
                    return null;
            }
