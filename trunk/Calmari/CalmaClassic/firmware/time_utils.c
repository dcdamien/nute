...
FORCE_INLINE void TimerInit (void) {
...
FORCE_INLINE bool TimerDelayElapsed (uint16_t *AVar, const uint16_t ADelay) {
    uint16_t ttmp;
...
FORCE_INLINE void TimerResetDelay (uint16_t *AVar) {
    ATOMIC_BLOCK (ATOMIC_RESTORESTATE) {
        *AVar = TickCounter;
    }
}
