...
FORCE_INLINE void Battery_Task (void) {
    ...
    switch (Battery.ADCState) {
        case ADCNoMeasure:
            if (DelayElapsed(&Battery.ADCTimer, BAT_MEASURE_PERIOD)) { // Check if timeout has passed
                // Start meter
                BATTERY_METER_ON();
        ...
        case ADCInit:
            ...
            if (ADC_MEASUREMENT_COMPLETED() && DelayElapsed (&Battery.ADCTimer, ADC_PREPARE_TIMEOUT)) {
