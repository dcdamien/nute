...
void CC_t::TransmitSync(rPkt_t *pPkt) {
...
void CC_t::TransmitAsync(rPkt_t *pPkt) {
    // WaitUntilChannelIsBusy();   // If this is not done, time after time FIFO is destroyed
    PWaitingThread = NULL;
 ...
 */
uint8_t CC_t::ReceiveSync(uint32_t Timeout_ms, rPkt_t *pPkt) {
    FlushRxFIFO();
...
// Return RSSI in dBm
int8_t CC_t::RSSI_dBm(uint8_t ARawRSSI) {
...
uint8_t CC_t::ReadFIFO(rPkt_t *pPkt) {
...
void CC_t::WriteTX(uint8_t* Ptr, uint8_t Length) {
    CsLo();                                                 // Start transmission
    BusyWait();                                             // Wait for chip to become ready
...
void CC_t::RfConfig(){
    ...
    WriteRegister(CC_PKTLEN,   RPKT_LEN);            // Packet length.
