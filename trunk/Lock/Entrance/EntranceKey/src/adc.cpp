...
void Adc_t::Init() {
    ...
    // ==== GPIO ====
    klGpioSetupByMsk(GPIOA, GPIO_Pin_0, GPIO_Mode_AIN);
    // ==== ADC's DMA ====
...
void Battery_t::Task() {
    ...
    Measure();
    BatteryState_t OldState = State;
    // Discharging slope
    if      ((State == bsFull) and (IValue < BATTERY_HALF))  State = bsHalf;
    else if ((State == bsHalf) and (IValue < BATTERY_EMPTY)) State = bsEmpty;
    // Charging slope
    else if ((State == bsHalf)  and (IValue > (BATTERY_HALF+60)))  State = bsFull;
    else if ((State == bsEmpty) and (IValue > (BATTERY_EMPTY+60))) State = bsHalf;
    // Rise evt if needed
    if(State != OldState) EvtNewState();
}
