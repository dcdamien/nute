...
static inline void klPinInFloating(GPIO_TypeDef *PGpioPort, const uint16_t APinNumber) {
    PGpioPort->ODR &= ~(1<<APinNumber);
    // Write needed bits to control reg
    if(APinNumber <= 7) {
        PGpioPort->CRL &= ~((uint32_t)0b1111 << (APinNumber * 4));  // clear previous bits
        PGpioPort->CRL |= ((uint32_t)pmInFloating << (APinNumber * 4));
    }
    else {
        PGpioPort->CRH &= ~((uint32_t)0b1111 << ((APinNumber - 8) * 4)); // clear previous bits
        PGpioPort->CRH |= ((uint32_t)pmInFloating << ((APinNumber - 8) * 4));
    }
}
