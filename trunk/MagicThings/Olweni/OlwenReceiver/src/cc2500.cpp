...
void CC_t::SetChannel(uint8_t AChannel) {
    ...
    while (InnerState != CC_STB_IDLE) EnterIdle();
    // Now set channel
...
// Return RSSI in dBm
int16_t CC_t::RSSI_dBm() {
    int16_t RSSI = RX_Pkt.RSSI;
    if (RSSI >= 128) RSSI -= 256;
    RSSI = (RSSI / 2) - 69;    // now it is in dBm
    return RSSI;
}
...
void CC_t::WriteStrobe (uint8_t AStrobe){
    CS_Lo();      // Start transmission
    BusyWait();   // Wait for chip to become ready
    InnerState = ReadWriteByte(AStrobe);  // Write strobe
    CS_Hi();      // End transmission
    InnerState &= 0b01110000; // Mask needed bits
}
