...
void PwmPin_t::Init(GPIO_TypeDef *GPIO, uint16_t N, TIM_TypeDef* PTim, uint8_t Chnl, uint16_t TopValue, bool Inverted) {
    ...
    if     (Tim == TIM1 ) { rccEnableTIM1(FALSE); }
    else if(Tim == TIM2 ) { rccEnableTIM2(FALSE); }
    else if(Tim == TIM3 ) { rccEnableTIM3(FALSE); }
#ifdef RCC_APB1ENR_TIM4EN
    else if(Tim == TIM4 ) { rccEnableTIM4(FALSE); }
#endif
    else if(Tim == TIM15) { rccEnableAPB2(RCC_APB2ENR_TIM15EN, FALSE); }
    else if(Tim == TIM16) { rccEnableAPB2(RCC_APB2ENR_TIM16EN, FALSE); }
    else if(Tim == TIM17) { rccEnableAPB2(RCC_APB2ENR_TIM17EN, FALSE); }

    // Clock src
    if(ANY_OF_4(Tim, TIM1, TIM15, TIM16, TIM17)) PClk = &Clk.APB2FreqHz;
