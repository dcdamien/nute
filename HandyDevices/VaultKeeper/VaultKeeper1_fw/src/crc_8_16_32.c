...
unsigned char crc8_tbl_buf(unsigned char * buf, unsigned char crc,unsigned long len)
{
	...
	/* No shifts here, since CRC8 is unsigned char */
...
unsigned short crc16_tbl_buf(unsigned char * buf, unsigned short crc,unsigned long len)
{
	...
	for (i = 0; i < len; i++)
	{
		crc = (crc << 8) ^ crc16_tbl[((crc>>8) ^ buf[i]) & 0xff];
/*
		crc = crc & 0xffff;
*/
	}
...
/************************************ CRC32 ************************************/
/* Note that 0x04C11DB7 is mirrored to 0xEDB88320 */
#define poly32 0xEDB88320
      

const unsigned long crc32_tbl [256]= {
