...
uint8_t i2cMgr_t::CmdPoll(I2C_Cmd_t ACmd) {
	...
	if ((rslt = SendStart()) == I2C_OK) {
		...
		i = 0;
		while((i++ < ACmd.DataToWrite.Length) and (rslt == I2C_OK)) {
			Uart.Printf("%u\r", i);
			I2C_SendData(I2C1, ACmd.DataToWrite.Buf[i]);
    ...
    if((rslt == I2C_OK) and (ACmd.Callback != 0)) ACmd.Callback();
